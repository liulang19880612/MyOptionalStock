...
class CGridCtrlDataSetObserver : public TObjRefImpl<ILvDataSetObserver>
{
public:
	CGridCtrlDataSetObserver(CGridCtrl *pView) :m_pOwner(pView)
	{
	}
...
//////////////////////////////////////////////////////////////////////////
void CGridCtrlDataSetObserver::onChanged()
{
	m_pOwner->onDataSetChanged();
}

void CGridCtrlDataSetObserver::onInvalidated()
{
	m_pOwner->onDataSetInvalidated();
}

void CGridCtrlDataSetObserver::OnItemChanged(int iItem)
{
	...
	, m_iPendingUpdateItem(-2)
{
	...
	m_observer.Attach(new CGridCtrlDataSetObserver(this));
...
BOOL CGridCtrl::SetAdapter(IMcAdapter * adapter)
{
	if (!m_lvItemLocator)
	{
...
int CGridCtrl::InsertColumn(int nIndex, LPCTSTR pszText, int nWidth, LPARAM lParam)
{
	SASSERT(m_pHeader);
...
BOOL CGridCtrl::CreateChildren(pugi::xml_node xmlNode)
{
	...
	while (pChild)
	{
		if (pChild->IsClass(CGridHeaderCtrl::GetClassName()))
		{
			m_pHeader = (CGridHeaderCtrl*)pChild;
			break;
		}
	...
	m_pHeader->GetEventSet()->subscribeEvent(EventHeaderItemChanging::EventID, Subscriber(&CGridCtrl::OnHeaderSizeChanging, this));
	m_pHeader->GetEventSet()->subscribeEvent(EventHeaderItemSwap::EventID, Subscriber(&CGridCtrl::OnHeaderSwap, this));
	m_pHeader->GetEventSet()->subscribeEvent(EventHeaderClick::EventID, Subscriber(&CGridCtrl::OnHeaderClick, this));

	return TRUE;
}


SOUI::CRect CGridCtrl::GetListRect()
{
	SOUI::CRect rcList;
...
//更新表头位置
void CGridCtrl::UpdateHeaderCtrl()
{
	SOUI::CRect rcClient;
	GetClientRect(&rcClient);
	SOUI::CRect rcHeader(rcClient);
...
void CGridCtrl::DeleteColumn(int iCol)
{
...
void CGridCtrl::UpdateChildrenPosition()
{
	__super::UpdateChildrenPosition();
	UpdateHeaderCtrl();
}

int CGridCtrl::GetHeaderHeight() const
{
	return m_nHeaderHeight.toPixelSize(GetScale());
}


bool CGridCtrl::OnHeaderClick(EventArgs *pEvt)
{
...
bool CGridCtrl::OnHeaderSizeChanging(EventArgs *pEvt)
{
	UpdateScrollBar();
	UpdateHeaderCtrl();
	SPOSITION pos = m_lstItems.GetHeadPosition();
	...
	{
		ItemInfo ii = m_lstItems.GetNext(pos);
		SOUI::CRect rcItem = ii.pItem->GetWindowRect();
		rcItem.right = m_pHeader->GetTotalWidth();
		ii.pItem->Move(rcItem);
		SOUI::CRect rcSubItem(rcItem);
...
bool CGridCtrl::OnHeaderSwap(EventArgs *pEvt)
{
...
void CGridCtrl::onDataSetChanged()
{
	if (!m_adapter) return;
	if (!IsVisible(TRUE))
	{
...
void CGridCtrl::onDataSetInvalidated()
{
	m_bDatasetInvalidated = TRUE;
	Invalidate();
}

void CGridCtrl::onItemDataChanged(int iItem)
{
	if (!m_adapter) return;
	if (!IsVisible(TRUE))
	{
...
void CGridCtrl::OnPaint(IRenderTarget *pRT)
{
	...
	if (iFirst != -1)
	{
		...
		for (; pos; i++)
		{
			...
			ItemInfo ii = m_lstItems.GetNext(pos);
			SOUI::CRect rcItem = _OnItemGetRect(iFirst + i);
...
BOOL CGridCtrl::OnScroll(BOOL bVertical, UINT uCode, int nPos)
{
...
void CGridCtrl::UpdateVisibleItems()
{
	if (!m_adapter) return;
		...
		while (pos < m_siVer.nPos + (int)m_siVer.nPage && iNewLastVisible < m_adapter->getCount())
		{
			...
			{//create new visible item
				...
				{//创建一个新的列表项
					...
					ii.pItem->GetEventSet()->subscribeEvent(EventItemPanelClick::EventID, Subscriber(&CGridCtrl::OnItemClick, this));
				}
				else
				{
			...
			SOUI::CRect rcItem(0, 0, m_pHeader->GetTotalWidth(), 100000);
			if (m_lvItemLocator->IsFixHeight())
			{
...
void CGridCtrl::UpdateVisibleItem(int iItem)
{
...
void CGridCtrl::OnSize(UINT nType, CSize size)
{
	...
	while (pos)
	{
		...
		SOUI::CRect rcItem(0, 0, m_pHeader->GetTotalWidth(), nHei);
		ii.pItem->Move(rcItem);
	}
...
//////////////////////////////////////////////////////////////////////////
void CGridCtrl::OnItemRequestRelayout(SItemPanel *pItem)
{
	//pItem->UpdateChildrenPosition();

}

BOOL CGridCtrl::IsItemRedrawDelay()
{
	return TRUE;
}

BOOL CGridCtrl::OnItemGetRect(SItemPanel *pItem, SOUI::CRect &rcItem)
{
...
SOUI::CRect CGridCtrl::_OnItemGetRect(int iPosition)
{
	int nOffset = m_lvItemLocator->Item2Position(iPosition) - m_siVer.nPos;
	SOUI::CRect rcItem = GetClientRect();
...
void CGridCtrl::OnItemSetCapture(SItemPanel *pItem, BOOL bCapture)
{
	if (bCapture)
	{
...
void CGridCtrl::RedrawItem(SItemPanel *pItem)
{
	pItem->InvalidateRect(NULL);
}

SItemPanel * CGridCtrl::HitTest(SOUI::CPoint & pt)
{
	SPOSITION pos = m_lstItems.GetHeadPosition();
	while (pos)
	{
		ItemInfo ii = m_lstItems.GetNext(pos);
		SOUI::CRect rcItem = ii.pItem->GetItemRect();
...
LRESULT CGridCtrl::OnMouseEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	SetMsgHandled(FALSE);
...
LRESULT CGridCtrl::OnKeyEvent(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRet = 0;
...
void CGridCtrl::OnKeyDown(TCHAR nChar, UINT nRepCnt, UINT nFlags)
{
	if (!m_adapter)
	{
		SetMsgHandled(FALSE);
...
void CGridCtrl::EnsureVisible(int iItem)
{
...
BOOL CGridCtrl::OnMouseWheel(UINT nFlags, short zDelta, SOUI::CPoint pt)
{
	SItemPanel *pSelItem = GetItemPanel(m_iSelItem);
	if (pSelItem)
	{
		SOUI::CRect rcItem = pSelItem->GetItemRect();
		SOUI::CPoint pt2 = pt - rcItem.TopLeft();
...
int CGridCtrl::GetScrollLineSize(BOOL bVertical)
{
	return m_lvItemLocator->GetScrollLineSize();
}

SItemPanel * CGridCtrl::GetItemPanel(int iItem)
{
...
void CGridCtrl::SetItemLocator(IListViewItemLocator *pItemLocator)
{
...
BOOL CGridCtrl::OnUpdateToolTip(SOUI::CPoint pt, SwndToolTipInfo & tipInfo)
{
	if (!m_pHoverItem)
...
void CGridCtrl::SetSel(int iItem, BOOL bNotify)
{
	if (!m_adapter) return;
...
void CGridCtrl::OnKillFocus(SWND wndFocus)
{
...
void CGridCtrl::OnSetFocus(SWND wndOld)
{
...
BOOL CGridCtrl::OnSetCursor(const SOUI::CPoint &pt)
{
	BOOL bRet = FALSE;
	if (m_itemCapture)
	{
	...
	{
		SOUI::CRect rcItem = m_pHoverItem->GetItemRect();
...
bool CGridCtrl::OnItemClick(EventArgs *pEvt)
{
...
void CGridCtrl::OnColorize(COLORREF cr)
{
...
void CGridCtrl::OnScaleChanged(int nScale)
{
...
HRESULT CGridCtrl::OnLanguageChanged()
{
...
void CGridCtrl::DispatchMessage2Items(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	SPOSITION pos = m_lstItems.GetHeadPosition();
	while (pos)
	{
...
void CGridCtrl::OnShowWindow(BOOL bShow, UINT nStatus)
{
