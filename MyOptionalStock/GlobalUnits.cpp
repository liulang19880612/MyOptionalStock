...
{
	std::string strPath = S_CW2A( GetExePath() + L"uires\\emojis\\*.png");
	struct _finddata_t fileinfo;
	auto handle = _findfirst(strPath.c_str(), &fileinfo);
	if(handle == -1)
		return;
	do
	{
		...
		SStringW sstrPath = GetExePath() + L"uires\\emojis\\" +  S_CA2W(fileinfo.name);
